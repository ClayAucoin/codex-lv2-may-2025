#!/bin/bash
# capstone-navigation.sh

# Usage: ./capstone-navigation.sh [folder_path] [file_prefix]
# Example: ./capstone-navigation.sh capstone-levels capstone-lv

# Default values for our capstone project
FOLDER_PATH=${1:-"capstone-levels"}
FILE_PREFIX=${2:-"capstone-lv"}
...
LESSON_NUMBERS=()
for file in "${LESSON_FILES[@]}"; do
    # Extract number from filename (e.g., "capstone-lv-5.md" -> "5")
    filename=$(basename "$file")
    # Remove the prefix and .md extension, then extract the number
