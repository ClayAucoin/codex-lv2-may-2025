...
→ Related: [Secret Variables](#secret-variables), [Postman](#postman)
# B
...
→ Related: [Bootstrap](#bootstrap), [Mobile-First](#mobile-first)
# C
...
### Configuration File
A file specifying initial settings for software.
→ Related: [Secret Variables](#secret-variables)
### Console Log
`console.log()` outputs values/messages to the browser’s developer console.
→ Related: [Debugging](#debugging)
### Container (Bootstrap)
A wrapper element in Bootstrap for layout control.
→ Related: [Bootstrap](#bootstrap)
### Counter Pattern
A programming pattern for incrementing or decrementing a variable (e.g., scores, loops).
...
→ Related: [Syntax Error](#syntax-error), [Console Log](#console-log)
### Destructure
...
→ Related: [git push](#git-push)
### git push
Sends local commits to remote repo.
→ Related: [git pull](#git-pull)
### git status
...
### ID Attribute
HTML attribute for unique element identifiers.
→ Related: [Class Attribute](#class-attribute)
### if Statement
...
### Mobile-First
Designing primarily for mobile, then scaling up.
...
### onEvent (Code.org)
Runs code when an event occurs on a UI element.
→ Related: [Event Handler](#event-handler)
### Opacity (CSS)
...
→ Related: [Function Signature](#function-signature)
...
### Postman
Tool for testing/documenting APIs.
→ Related: [API Token](#api-token)
### Prompt Engineering
Crafting prompts for AI to optimize responses.
→ Related: [AI Chatbot](#ai-chatbot)
### Properties (Programming)
Attributes of objects/elements.
→ Related: [Sprite](#sprite)
# S
...
### Secret Variables
Hidden environment variables (e.g., API keys).
→ Related: [API Token](#api-token), [Configuration File](#configuration-file)
### Self-Paced
...
### Variables
Named storage for values in programs.
→ Related: [Global Variable](#global-variable), [Counter Pattern](#counter-pattern)
